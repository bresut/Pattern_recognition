...
def pred3(image_arr, boxes, scores, HU_min, HU_max, open_radius, close_radius, gaus, f_name, f_dir, ai_label, img_of_box,arrs):
    ...
    for i in range(boxes.shape[0]):#boundingbox數次
        if scores is None or scores[i] > min_thresh:#沒有scores或大於minthresh
            ...
            find_coordinate(iou_f_name, i, xmin, xmax, ymin, ymax,f_name)
            #############胰腺HU value 範圍
...
def find_coordinate(iou_f_name, i, xmin, xmax, ymin, ymax, f_name):
    try:#使用try exception防止找不到xml error
        #檔案載入並解析xml資料
        ...
        tree = ET.parse('../../Data/'+ case_number + "/" + detection_type +'/xml/'+case_number+"/"+case_number+"_"+iou_f_name)
        # 從字串中取得並解析xml資料
        root = tree.getroot()
...
def image_inference(gauge_count, gauge):
    ...
    print('Done!')
    PATH_TO_TEST_FILE='../../Data/'+ case_number + "/" + detection_type +'/testmodel/' + case_number
    ...
    for IMG_PATH in IMGS_FOR_TEST:
        print(IMG_PATH)
        end =  int(IMG_PATH.split('-')[2].split('.')[0])
...
def F1score(num_ai_detection, ai_label, img_of_box, count_all_images,arrs,end_number):
    xmlpath = '../../Data/'+ case_number + "/" + detection_type +'/xml/'+case_number    #獲取當前路徑
    ...
    for fn in os.listdir(xmlpath):#計算TP
        ...
        for label in second_check:
            if fn != '@eaDir' and fn.split('.')[1] =='xml' and fn.split('-00')[2].split('.')[0] == label[0].split('-00')[2]:
...
def enhence_show(second_check):
    PATH_TO_JPG='../../Data/'+ case_number + "/" + detection_type +'/testmodel/' + case_number
    ...
    for XML_PATH in XMLS_FOR_TEST:
        ...
        PATH_TO_JPG='../../Data/'+ case_number + "/" + detection_type +'/testmodel/' + case_number + "/" + f_name
...
def xmlshow():
    PATH_TO_JPG='../../Data/'+ case_number + "/" + detection_type +'/xml/' + case_number
    XMLS_FOR_TEST = testImgsFilePaths(PATH_TO_JPG)
    for XML_PATH in XMLS_FOR_TEST:
        f_name = os.path.split(XML_PATH)[1]
        img_num = f_name.split('_')[1]
        iou_f_name = os.path.split(XML_PATH)[1].split('.')[0]+".xml"
        tree = ET.parse('../../Data/'+ case_number + "/" + detection_type +'/xml/'+case_number+"/"+iou_f_name)
        # 從字串中取得並解析xml資料
        root = tree.getroot()
        counter = 0
        for count in root.iter('object'):
...
def creat_xml(second_check):
    ...
    for i in range(len(second_check)):
        if second_check[i][0] != second_check[i-1][0]:
            ...
            if not os.path.exists(path):
                os.makedirs(path)
            tree.write('../../Data/'+ case_number + "/" + detection_type +'/pancreas_xml/' + second_check[i][0] +'.xml',  encoding='utf-8')
        else:
            print(second_check[i][0])
...
def edit_xml(second_check):
    ...
    tree = ET.parse('../../Data/'+ case_number + "/" + detection_type +'/pancreas_xml/' + second_check[0] +'.xml')
    root = tree.getroot()
    ...
    #print('error5')
    tree.write('../../Data/'+ case_number + "/" + detection_type +'/pancreas_xml/' + second_check[0] +'.xml',method='xml',encoding='utf-8')       # 更新xml文件
